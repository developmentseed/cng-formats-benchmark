...
class FlatGeobufParams(BaseModel):
    ...
    * ``"sentinel"`` keeps every feature, substituting a minimal placeholder
      ...
      (near-free) NULL representation.
    """

    model_config = ConfigDict(extra="ignore")

    spatial_index: bool = True
    null_geometry: Literal["error", "drop", "sentinel"] = "error"
...
def describe_flatgeobuf_layout(
    ...
    *,
    ...
    ``features_dropped``/``features_sentinel`` cannot be recovered from ``path``
    alone (the file only knows what it contains, not what was excluded or
    substituted), so the caller — the adapter that just wrote it — passes them
    through from its own ``null_geometry`` accounting (#98).
    """
...
def _substitute_sentinel_geometries(gdf, null_mask) -> Any:
    ...
    geometry" by construction, without dropping the row or its attributes. A
    boolean flag column (named ``null_geometry``, or a collision-safe variant)
    records which rows are placeholders.
    """
    out = gdf.copy()
...
def _apply_null_geometry_policy(gdf, opts: FlatGeobufParams) -> tuple[Any, int, int]:
    ...
    * ``"sentinel"`` keeps every feature behind a placeholder geometry (see
      ...
      records how many for the result to say so.
    * ``"error"`` (the default) only raises when the index would actually fail
    ...
    if null_count == total:
        raise ValueError(
            ...
            "real geometry to index, drop around, or infer a sentinel type from."
        )

    if opts.null_geometry == "drop":
        return gdf.loc[~null_mask].reset_index(drop=True), null_count, 0

    if opts.null_geometry == "sentinel":
        return _substitute_sentinel_geometries(gdf, null_mask), 0, null_count
    ...
    if opts.spatial_index:
        ...
        raise ValueError(
            ...
            "params.null_geometry: sentinel to keep every feature behind a "
            "placeholder geometry (recorded on the layout, and excluded from "
            "spatial queries by construction), or params.spatial_index: false "
            "to keep every feature without the index."
        )
    return gdf, 0, 0
...
class FlatGeobufAdapter(FormatAdapter):
    ...
    def __init__(self) -> None:
        # A produced file cannot say how many features were excluded or
        # sentinel-substituted — that accounting only exists at write time —
        # so :meth:`convert` records it here, keyed by target path, for
        # :meth:`describe_layout` to pick up right after (#98). The runner
        # always calls the two in sequence for the same object before
        # converting anything else through this instance.
        self._null_geometry_stats_by_target: dict[str, tuple[int, int]] = {}
    ...
    def convert(self, source: str, target: str, params: dict[str, Any]) -> None:
        ...
        gdf, dropped, sentinel = _apply_null_geometry_policy(gdf, opts)
        self._null_geometry_stats_by_target[target] = (dropped, sentinel)
    ...
    def describe_layout(
        ...
        dropped, sentinel = self._null_geometry_stats_by_target.get(target, (0, 0))
        return [
