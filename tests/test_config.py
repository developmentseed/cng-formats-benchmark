...
def test_example_s2_display_comparison_trio_shares_target_resolutions():
    ...
    geozarr_cfg = load_benchmark_config(MAJA_GEOZARR_BENCHMARK_EXAMPLE)
    reader_cfg = load_benchmark_config(
        "configs/benchmarks/example_sentinel2_maja_geozarr_reader.yaml"
    )
    trio = [cog_cfg, geozarr_cfg, reader_cfg]
    for cfg in trio:
    ...
    assert (
        ...
        == reader_cfg.params["display_target_resolutions"]
    )


def test_example_s1_display_comparison_trio_shares_target_resolutions():
    cog_cfg = load_benchmark_config(
        "configs/benchmarks/example_sentinel1_rtc_cog_zstd.yaml"
    )
    geozarr_cfg = load_benchmark_config(
        "configs/benchmarks/example_sentinel1_rtc_geozarr.yaml"
    )
